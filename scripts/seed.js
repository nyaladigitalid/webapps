...
async function run() {
  ...
  try {
    ...
    // Helper to get or insert ID
    const getOrInsert = async (table, checkCol, checkVal, insertSql, insertParams) => {
      const [rows] = await conn.execute(`SELECT id FROM ${table} WHERE ${checkCol} = ?`, [checkVal]);
      if (rows.length > 0) return { insertId: rows[0].id };
      const [res] = await conn.execute(insertSql, insertParams);
      return res;
    };

    const u1 = await getOrInsert(
      'users', 'email', 'admin@nyala.local',
      'INSERT INTO users (name,email,role,password_hash) VALUES (?,?,?,?)',
      ['Admin Nyala', 'admin@nyala.local', 'admin', null]
    );
    const u2 = await getOrInsert(
      'users', 'email', 'ops1@nyala.local',
      'INSERT INTO users (name,email,role,password_hash) VALUES (?,?,?,?)',
      ['Operator Satu', 'ops1@nyala.local', 'operator', null]
    );

    const c1 = await getOrInsert(
      'clients', 'whatsapp', '628123456789',
    ...
    const c2 = await getOrInsert(
      'clients', 'whatsapp', '628987654321',
      'INSERT INTO clients (name,business_name,business_type,whatsapp,address) VALUES (?,?,?,?,?)',
      ['Green Kitchen', 'Green Kitchen', 'Catering', '628987654321', 'Jl. Melati No. 5, Jakarta']
    );

    const p1 = await getOrInsert(
      'packages', 'code', 'PKT-SOCIAL',
    ...
    const p2 = await getOrInsert(
      'packages', 'code', 'PKT-SEO',
      'INSERT INTO packages (code,name,price_monthly,description,active) VALUES (?,?,?,?,?)',
      ['PKT-SEO', 'SEO Booster', 1200000, 'Optimasi SEO bulanan', 1]
    );

    // Orders logic: check if exists by client_id + package_id + status (simple check to avoid dupes on re-run)
    // For simplicity, we just check if ANY order exists for this client+package. If yes, skip insert.
    const getOrder = async (clientId, packageId, insertSql, insertParams) => {
        const [rows] = await conn.execute('SELECT id FROM orders WHERE client_id=? AND package_id=? LIMIT 1', [clientId, packageId]);
        if (rows.length > 0) return { insertId: rows[0].id, skipped: true };
        const [res] = await conn.execute(insertSql, insertParams);
        return { insertId: res.insertId, skipped: false };
    };

    const o1 = await getOrder(
      c1.insertId, p1.insertId,
